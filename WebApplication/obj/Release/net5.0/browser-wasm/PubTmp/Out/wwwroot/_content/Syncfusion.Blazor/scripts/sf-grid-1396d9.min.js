...
window.sfBlazor.Grid = function () {
  ...
  function () {
    ...
    Scroll.prototype.removeUnwantedScroll = function (offsetValue) {
      ...
      if (this.parent.options.frozenColumns != 0) {
        isFrozenColumn = this.getMovableContent().offsetWidth >= this.getMovableContentTable().offsetWidth - 2;
      }

      if (offsetValue === "Height" && this.parent.content.offsetHeight >= this.parent.getContentTable().offsetHeight) {
        return true;
      }
  ...
  function () {
    ...
    Freeze.prototype.setFrozenHeight = function () {
      ...
      if (this.parent.options.enableVirtualization && this.parent.options.enableColumnVirtualization && frozenscrollbarX) {
        var columns = this.parent.options.columns;
        var totalColumnWidth = 0;

        for (var i = 0; i < columns.length; i++) {
          totalColumnWidth += parseInt(columns[i].width.split ? columns[i].width.split("px")[0] : columns[i].width.toString(), 10);
        }

        movableChildScrollBarX.style.width = totalColumnWidth - frozenColumnWidth + getScrollBarWidth() + "px";
      } else {
      ...
      this.parent.scrollModule.getScrollbar().style.display = this.parent.scrollModule.removeUnwantedScroll("Width") ? "none" : "flex"; //if (movableContent.scrollWidth - movableContent.clientWidth) {
    ...
    function HeaderDragDrop(parent) {
      ...
      this.drag = function (e) {
        ...
        if (target) {
          ...
          var contentElement = sf.base.closest(target, '.e-content');
  ...
  function () {
    ...
    Resize.prototype.resizeColumn = function (fName, index, id) {
      ...
      columnIndexByField = columnIndexByField === -1 ? this.parent.getColumnIndexByUid(fName) : columnIndexByField;
    ...
    Resize.prototype.findColumn = function (fName) {
      var _loop_1 = function _loop_1(i) {
        ...
        columnIndex = columnIndex === -1 ? this_1.parent.getColumnIndexByUid(fieldName) : columnIndex;
  ...
  function () {
    ...
    Group.prototype.columnDrag = function (e) {
      ...
      if (parentsUntil(e.target, 'e-groupdroparea') && !column.allowReordering) {
        sf.base.classList(cloneElement, ['e-defaultcur'], ['e-notallowedcur']);
      }
  ...
  function () {
    ...
    Selection.prototype.mouseDownHandler = function (e) {
      ...
      if (parentsUntil(target, 'e-rowcell') && !e.shiftKey && !e.ctrlKey) {
        if ((gObj.options.cellSelectionMode.indexOf('Box') > -1 || (gObj.options.allowDragSelection && gObj.options.cellSelectionMode.indexOf('Flow')) > -1) && !this.isRowType() && !this.isSingleSel()) {
        ...
        if ((gObj.options.allowRowDragAndDrop || gObj.options.allowDragSelection) && !gObj.options.isEdit) {
          if (!gObj.options.allowDragSelection && !this.isRowType() || this.isSingleSel() || sf.base.closest(target, 'td').classList.contains('e-selectionbackground')) {
    ...
    Selection.prototype.mouseUpHandler = function (e) {
      ...
      } else if (this.isDragged && !this.isAutoFillSel && (!sf.base.isNullOrUndefined(this.startDragIndex) || !sf.base.isNullOrUndefined(this.endDragIndex) || !sf.base.isNullOrUndefined(this.endDragCellIndex))) {
        this.isDragged = false;
    ...
    Selection.prototype.enableDrag = function (e, isUpdate) {
      ...
      if (isUpdate) {
        ...
        this.startDragIndex = parseInt(tr.getAttribute('aria-rowindex'), 10);
        this.startDragCellIndex = parseInt(parentsUntil(e.target, 'e-rowcell').getAttribute('aria-colindex'), 10);
      }
    ...
    Selection.prototype.mouseMoveHandler = function (e) {
      ...
      if (!this.isCellDrag || this.parent.options.allowDragSelection && this.isCellDrag) {
        if (!target) {
      ...
      if (target && !e.ctrlKey && !e.shiftKey) {
        ...
        if (!this.isCellDrag && (sf.base.isNullOrUndefined(this.prevStartDIndex) || this.prevStartDIndex != this.startDragIndex || this.prevEndIndex != rowIndex)) {
        ...
        } else if (this.parent.options.allowDragSelection && this.isCellDrag) {
          var target_1;
          var isInvokedFirst = false;
          ...
          if (target_1) {
            ...
            this.endDragCellIndex = parseInt(target_1.getAttribute('aria-colindex'), 10);

            if (!this.isAutoFillSel) {
              if (isInvokedFirst) {
                this.parent.dotNetRef.invokeMethodAsync("DragCellSelection", this.startDragIndex, this.startDragCellIndex, rowIndex, parseInt(target_1.getAttribute('aria-colindex'), 10), true);
              } else {
                this.performDragCellSelection(this.startDragIndex, this.startDragCellIndex, rowIndex, parseInt(target_1.getAttribute('aria-colindex'), 10));
              }
            }
          }
    ...
    Selection.prototype.selectCellsByRange = function (sIndex, eIndex, startCellIndex, endCellIndex) {
      ...
      for (var i = 0; i < rows.length; i++) {
        var row = rows[i];
        var cells_1 = [].slice.call(row.querySelectorAll('.e-rowcell'));
      ...
      for (var i = sIndex; i <= eIndex; i++) {
        ...
        for (var j = min < max ? min : max, len = min > max ? min : max; j <= len; j++) {
          cells.push(rows[i].querySelectorAll('.e-rowcell')[j]);
        }
    ...
    Selection.prototype.getLastColIndex = function (rowIndex) {
      var cells = this.parent.options.frozenColumns ? this.parent.getMovableDataRows()[rowIndex].querySelectorAll('td.e-rowcell') : this.parent.getDataRows()[rowIndex].querySelectorAll('td.e-rowcell');
    ...
    Selection.prototype.selectRangeOfRows = function (startIndex, endIndex) {
      ...
      var _loop_1 = function _loop_1(i) {
        if (!sf.base.isNullOrUndefined(movableRows) && movableRows.length > 0) {
          var movableRow = this_1.parent.options.enableVirtualization ? movableRows.filter(function (_) {
            return parseInt(_.getAttribute("aria-rowindex"), 10) == i;
          })[0] : movableRows[i];
          this_1.selectRow(movableRow, true);
        }
    ...
    Selection.prototype.clearSelectionExceptDragIndexes = function (startIndex, endIndex) {
      ...
      for (var i = 0; i < rows.length; i++) {
        var j = this.parent.options.enableVirtualization ? parseInt(rows[i].getAttribute('aria-rowindex'), 10) : i;

        if (j < startIndex || j > endIndex) {
          if (!sf.base.isNullOrUndefined(movableRows) && movableRows.length > 0) {
            var movableRow = this.parent.options.enableVirtualization ? movableRows.filter(function (_) {
              return parseInt(_.getAttribute("aria-rowindex"), 10) == j;
            })[0] : movableRows[j];
            this.clearSelectionByRow(movableRow);
          }
  ...
  function () {
    ...
    VirtualContentRenderer.prototype.onDataReady = function () {
      ...
      if (!this.movableColumnIndex && this.parent.options.frozenColumns) {
        this.movableColumnIndex = parseInt(this.parent.scrollModule.getMovableContent().querySelector('td').getAttribute('aria-colindex'));
      }
  ...
  function () {
    ...
    SfGrid.prototype.getSelectedRowIndexes = function () {
      ...
      for (var i = 0; i < rows.length; i++) {
        if (rows[i].hasAttribute('aria-selected') && rows[i].getAttribute('aria-selected') === "true") {
          var rowIndex = this.options.allowDragSelection && this.options.enableVirtualization ? parseInt(rows[i].getAttribute('aria-rowindex'), 10) : i;
    ...
    */
    ...
    SfGrid.prototype.targetIsFilterDialog = function (e) {
      ...
      if (popupElement || filterDropdown || filterCheckbox || searchClear || datetimePicker || daterangePicker || isBlankCheckbox || ccPopupElement) {
        return true;
      } else {
        return false;
      }
  ...
  var Grid = {
    ...
    preventResizeAction: function preventResizeAction(element, isCancel) {
      if (!sf.base.isNullOrUndefined(element) && !sf.base.isNullOrUndefined(element.blazor__instance)) {
        var instance = element.blazor__instance;
    ...
    setMediaColumns: function setMediaColumns(element, isResetPersistData) {
      if (!sf.base.isNullOrUndefined(element) && !sf.base.isNullOrUndefined(element.blazor__instance)) {
        element.blazor__instance.columnChooserModule.setMediaColumns(isResetPersistData);
      }
    },
    ...
    scrollIntoView: function scrollIntoView(element, columnIndex, rowIndex, rowHeight) {
      var gObj = element.blazor__instance;
      var scrollContent = gObj.options.frozenColumns ? gObj.scrollModule.getMovableContent() : gObj.getContent();

      if (gObj.options.enableVirtualization && rowIndex != -1) {
