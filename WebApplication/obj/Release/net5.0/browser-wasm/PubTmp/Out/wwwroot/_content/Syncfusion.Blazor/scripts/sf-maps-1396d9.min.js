...
window.sfBlazor.Maps = function () {
  ...
  function () {
        ...
        if (id.indexOf('_LayerIndex_') > -1) {
          ...
          if (id.indexOf('shapeIndex') > -1) {
            ...
            if (id.indexOf('shapeIndex') > -1 && this.options.layerHighlightSettings[layerIndex] && this.options.layerHighlightSettings[layerIndex].Enable) {
              ...
              if (!this.targetElementPath.classList.contains('mapShapeSelection')) {
    ...
    SfMaps.prototype.keyDown = function (event) {
      ...
      if (event.key === '+' || event.code === 'Equal') {
        ...
        zoomType = 'Reset';
        this.scaleFactor = this.options.minZoom;
        this.removeCluster();
        this.dotNetRef.invokeMethodAsync('TriggerZoom', zoomType, eventType);
        ...
        this.clientX = 0;
        this.panning(this.clientX, event.code === 'ArrowUp' ? -this.moveClientY : this.moveClientY, event['layerX'], event['layerY'], this.scaleFactor);
        ...
        this.panning(event.code === 'ArrowLeft' ? -this.moveClientX : this.moveClientX, this.clientY, event['layerX'], event['layerY'], this.scaleFactor);
        this.dotNetRef.invokeMethodAsync('UpdateTranslatePoint', this.isTileMap ? this.translatePoint : this.shapeTranslatePoint, this.tileTranslatePoint, this.scaleFactor);
      ...
      this.toolbarButtonColor(factor, id);
      this.clientX = this.moveClientX;
      this.clientY = this.moveClientY;
    };
    ...
    SfMaps.prototype.mouseUp = function (event) {
      ...
      if (!(id.indexOf('_Zooming_') > -1) && this.options.enableZoom && !this.isPanning && event.type !== 'mouseleave' && (this.options.zoomOnClick || this.options.doubleClickZoom && event.type === 'touchend' && this.tapCount > 1)) {
        this.tapCount = 0;
        var factor = this.scaleFactor + 1;
      ...
      if (this.allowPanning && this.isPanning && this.options['enablePanning']) {
        ...
        if (this.isTileMap && !sf.base.isNullOrUndefined(this.currentTiles) && !sf.base.isNullOrUndefined(animatedTiles) && this.currentTiles.childElementCount != animatedTiles.childElementCount && this.currentTiles.childElementCount < animatedTiles.childElementCount) {
          for (var i = animatedTiles.childElementCount - 1; i >= this.currentTiles.childElementCount; i--) {
            animatedTiles.removeChild(animatedTiles.childNodes[i]);
          }
        }
      ...
      if (!(id.indexOf('_Zooming_') > -1) && event.type !== 'mouseleave' && !this.isPinchZoomed) {
    ...
    SfMaps.prototype.mouseDown = function (event) {
      ...
      this.toolbarButtonColor(finalFactor, id);
    };

    SfMaps.prototype.toolbarButtonColor = function (finalFactor, id) {
    ...
    SfMaps.prototype.highlightStyle = function (style, className, setting, elementId) {
      if (elementId.indexOf('NavigationIndex') > -1) {
        ...
        if (this.navigation[layerIndex][lineIndex].Angle === 0) {
          style.innerHTML = '.' + className + ' {fill:' + setting['Fill'] + ';fill-opacity:' + setting['Opacity'] + ';stroke:' + setting['BorderColor'] + ';stroke-width:' + setting['BorderWidth'] + ';stroke-opacity:' + setting['BorderOpacity'];
        } else {
          style.innerHTML = '.' + className + ' {fill: none;fill-opacity:' + setting['Opacity'] + ';stroke:' + setting['Fill'] + ';stroke-width:' + setting['BorderWidth'] + ';stroke-opacity:' + setting['BorderOpacity'];
        }
      } else {
        style.innerHTML = '.' + className + ' {fill:' + setting['Fill'] + ';fill-opacity:' + setting['Opacity'] + ';stroke:' + setting['BorderColor'] + ';stroke-width:' + setting['BorderWidth'] + ';stroke-opacity:' + setting['BorderOpacity'];
      }
    ...
    SfMaps.prototype.generateTiles = function () {
      ...
      for (var l = tiles.length; l < animatedElement.childNodes.length; l++) {
        var isExistingElement = false;

        for (var a = 0; a < this.currentTiles.childElementCount; a++) {
          if (!isExistingElement && this.currentTiles.children[a].id === animatedElement.childNodes[l].id) {
            isExistingElement = true;
          }
        }

        if (isExistingElement) {
          animatedElement.childNodes[l].style.display = 'none';
        } else {
          animatedElement.removeChild(animatedElement.childNodes[l]);
        }
      }
    };

    SfMaps.prototype.getBingMap = function (tile, imageUrl) {
      var quadKey = ''; // eslint-disable-next-line prefer-const
    ...
    SfMaps.prototype.clusterSeparate = function (sameMarkerData, markerElement, isDom, id, connectorLine) {
      ...
      for (var i = 0; i < markerDataLength; i++) {
        ...
        this.clusterLocation.push({
          x: bounds['left'] - containerBound['left'],
          y: bounds['top'] - containerBound['top']
        });
    ...
    SfMaps.prototype.shapeHighlightHandle = function (fill, opacity, layerIndex) {
      ...
      this.handleHighlight(this.targetElementPath, 'Shape', {
        ...
        BorderWidth: highlighProperty['BorderWidth'],
    ...
    SfMaps.prototype.itemHighlightHandle = function (fill, opacity, layerIndex, settingIndex, type) {
      ...
      if (type === 'Navigation') {
        ...
        this.handleHighlight(this.targetElementPath, 'Navigation', {
              ...
              if (markerTemplate.children[o]['style']['visibility'] !== 'hidden' && markerIndex === 0) {
                ...
                if (bounds1_1 !== null) {
                  ...
                  if (colloideBounds.length > 0) {
                    ...
                    var clusterID = id + '_LayerIndex_' + layerIndex_2 + '_MarkerIndex_' + markerIndex + '_dataIndex_' + dataIndex + '_cluster_' + m;
                    var labelID = id + '_LayerIndex_' + layerIndex_2 + '_MarkerIndex_' + markerIndex + '_dataIndex_' + dataIndex + '_cluster_' + m + '_datalabel_' + m;
                    m++;
                    var shape = void 0;
  ...
  var Maps = {
    ...
    markerCluster: function markerCluster(element) {
      if (element && element.blazor__instance) {
    ...
    itemHighlight: function itemHighlight(fill, opacity, layerIndex, settingIndex, type, element) {
      if (element && element.blazor__instance) {
        element.blazor__instance.itemHighlightHandle(fill, opacity, layerIndex, settingIndex, type);
      }
    },
    shapeHighlight: function shapeHighlight(fill, opacity, layerIndex, element) {
      if (element && element.blazor__instance) {
    ...
    updateTileTranslatePoint: function updateTileTranslatePoint(tileTranslatePoint, translatePoint, areaRectX, areaRectY, areaRectWidth, areaRectHeight, marginLeft, marginTop, tileZoomLevel, element) {
      if (element && element.blazor__instance) {
    ...
    updateTranslatePoint: function updateTranslatePoint(translatePoint, areaRectX, areaRectY, areaRectWidth, areaRectHeight, marginLeft, marginTop, zoomFactor, element) {
      if (element && element.blazor__instance) {
    ...
    updateZoomSettings: function updateZoomSettings(element, zoomSettings) {
      if (element && element.blazor__instance) {
    ...
    markerAnimation: function markerAnimation(id, marker, dotNetRef, element) {
      if (element && element.blazor__instance) {
        element.blazor__instance.markerAnimation(id, marker, dotNetRef);
      }
    },
    layerAnimation: function layerAnimation(id, point, scale, currentPoint, currentScale, dotNetRef, element) {
    ...
    bubbleAnimation: function bubbleAnimation(id, bubble, dotNetRef, element) {
      if (element && element.blazor__instance) {
        element.blazor__instance.bubbleAnimation(id, bubble, dotNetRef);
      }
    },
    getToolbarAlign: function getToolbarAlign(id, data, element) {
